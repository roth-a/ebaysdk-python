...
class Connection(BaseConnection):
    ...
    def build_request_headers(self, api_call_name, url, body):
        if not self.signature_service:
            self.signature_service = SignatureService(token=self.config.values["token"])
        ...
        headers.update(self.signature_service.get_signature_dict(url, body))
        return headers
    ...
    def _get_resp_body_errors(self):
        ...
        for e in dom.findall("Errors"):
            ...
            msg = str(
                "Class: {eClass}, Severity: {severity}, Code: {code}, {shortMsg} {longMsg}"
            ).format(
                eClass=eClass,
                severity=eSeverity,
                code=eCode,
