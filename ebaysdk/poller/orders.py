...
from ebaysdk.poller import file_lock, parse_args
        ...
        if order.ShippingDetails.get("ShipmentTrackingDetails", None):
            data.extend(
                [
                    ...
                    (
                        "Tracking",
                        order.ShippingDetails.ShipmentTrackingDetails.ShipmentTrackingNumber,
                    ),
                    (
                        "Carrier",
                        order.ShippingDetails.ShipmentTrackingDetails.ShippingCarrierUsed,
                    ),
                    (
                        "Cost",
                        (
                            order.ShippingDetails.ShippingServiceOptions.ShippingServiceCost._currencyID,
...
class Poller(object):
    ...
    def run(self):
        ...
        with file_lock("/tmp/.ebaysdk-poller-orders.lock"):
            ...
            from_time = to_time - timedelta(
                hours=self.opts.hours, minutes=self.opts.minutes
            )
