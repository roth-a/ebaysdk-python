...
def init_options():
    ...
    parser.add_option(
        "-s", "--store_name", dest="store_name", default=None, help="Store name"
    )
    parser.add_option(
        "-f",
        "--file",
    ...
    parser.add_option(
        "-o", "--offset", dest="offset", default=0, help="Input file line offset."
    )
    parser.add_option(
        "-e", "--line_end", dest="line_end", default=None, help="Input file lines."
    )
    parser.add_option(
        "-n",
        "--domain",
...
def record_exists(store_name):
    query = {
        "query": {"match": {"store_name": {"query": store_name, "type": "phrase"}}}
    }

    query_req = requests.get(
        "http://elastic-1-2467465.lvs02.dev.ebayc3.com:9200/stores/storeMeta/_search",
...
def get_store_meta(store_name):
    ...
    try:
        ...
        data = {
            ...
            "seller_name": response.reply.searchResult.item[
                0
            ].sellerInfo.sellerUserName,
            "store_url": response.reply.searchResult.item[0].storeInfo.storeURL,
            "feedback_score": response.reply.searchResult.item[
                0
            ].sellerInfo.feedbackScore,
            "positive_feedback_percent": response.reply.searchResult.item[
                0
            ].sellerInfo.positiveFeedbackPercent,
            "top_rated_seller": response.reply.searchResult.item[
                0
        ...
        for category_node in agg_data["L1"]:
            ...
            try:
                while True:
                    ...
                    analyze_items(
                        category_call2.reply.searchResult.item, category_id, agg_data
                    )
